...
/**
 * Servlet implementation class CollectDelete
 */
@WebServlet("/collectdelete")
public class CollectDelete extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		web_animal_collect collect = web_animal_collectDao.selectCollect(uid, id, state);

		//System.out.println(collect);

		// uid用户名，id为被收藏ID，state用于区分动物和纪念品，此处为收藏纪念品，为2，当查询到此纪录，返回对象
		int count = web_animal_collectDao.delcollect(collect.getCollect_id());
		if (count > 0 && state == 2) {
			// 用户成功取消收藏纪念品，进行跳转
